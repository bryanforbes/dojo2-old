define(["../has", "../dom"], function(has, dom){
"use strict";
// summary:
...
var liteEngine = function(selector, root){
	...
	if(match){
		...
		if(match[2]){
			// an #id
			// use dom.byId as it fixes the id retrieval in IE
			found = dom.byId(match[2]);
...
if(!has("dom-matches-selector")){
	jsMatchesSelector = (function(){
		...
		function className(name){
			var classNameSpaced = ' ' + name + ' ';
			return function(node){
				return node.className.indexOf(name) > -1 && (' ' + node.className + ' ').indexOf(classNameSpaced) > -1;
			};
		}
		var attrComparators = {
